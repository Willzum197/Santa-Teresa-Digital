...
# ============================================
# PANEL ADMIN - CONTENIDO COMPLETO CON CREAR, MODIFICAR Y ELIMINAR
# ============================================
if st.session_state.get('es_admin', False):
    ...
    if "📰 Noticias" in admin_opt:
        ...
        # Lista con opciones MODIFICAR y ELIMINAR
        ...
        if not personajes.empty:
            for _, p in personajes.iterrows():
                with st.expander(f"👤 {p['nombre']} - {p['fecha']}"):
                    ...
                    with col1:
                        if st.button(f"✏️ Modificar", key=f"edit_per_{p['id']}"):
                            st.session_state.edit_personaje = p.to_dict()
                            st.rerun()
                    with col2:
                        if st.button(f"🗑️ Eliminar", key=f"del_per_{p['id']}"):
